...
class AuthViewModel: ObservableObject {
    ...
    func signIn(email: String, password: String) async {
        ...
        do {
            let user = try await authManager.auth.signUpUser(email: email, password: password)
            try await authManager.handleSignInSuccess(firebaseUser: user, fullName: user.displayName ?? "Unknown", email: email)
            success = true
        } catch let error as AuthError {
            errorMessage = AuthErrorMessage(message: error.errorDescription ?? "Oops, there was an error")
        } catch {
    ...
    func signUp(email: String, password: String, fullName: String, confirmPassword: String) async {
        ...
        guard password == confirmPassword else {
            errorMessage = AuthErrorMessage(message: "Passwords must match")
            return
        }
        
        guard startRequest() else { return }
        do {
            let user = try await authManager.auth.sigInUser(email: email, password: password)
            try await authManager.handleSignInSuccess(firebaseUser: user, fullName: user.displayName ?? "Unknown", email: user.email!)
        } catch let error as AuthError {
            errorMessage = AuthErrorMessage(message: error.errorDescription ?? "Oops, there was an error")
        } catch {
            errorMessage = AuthErrorMessage(message: "An unexpected error occurred")
    ...
    func signInWithGoogle() async{
        guard startRequest() else { return }
        do {
            let user = try await authManager.auth.signInWithGoogle()
            try await authManager.handleSignInSuccess(firebaseUser: user, fullName: user.displayName ?? "Unknown", email: user.email!)
            success = true
        } catch {
    ...
    func signInWithApple() async {
        guard startRequest() else { return }
        do {
            let user = try await authManager.auth.signInwithApple()
            try await authManager.handleSignInSuccess(firebaseUser: user, fullName: user.displayName ?? "Unknown", email: user.email!)
            success = true
        } catch {
